...
--   - Rebuild views safely (drop+create views, NO CASCADE; drop dependents first)
-- =============================================================================

begin;

-- -----------------------------------------------------------------------------
-- 0) Drop dependent view first (NO CASCADE)
-- -----------------------------------------------------------------------------
drop view if exists public.v_production_decisions_explain;

-- -----------------------------------------------------------------------------
-- 1) Canonical classifier
--    Signature must already exist and MUST KEEP PARAMETER NAMES STABLE.
...
as $$
  with m as (
    select
      ...
      -- noise-resistant baseline and acceleration ratio
  ...
  select
    case
      ...
      -- INCIDENT: high severity + recent activity OR sharp acceleration
      ...
      -- INVESTIGATE: meaningful, baseline-confirmed elevation
      when
...
-- -----------------------------------------------------------------------------
-- 1b) Compatibility overload
-- IMPORTANT: Keep the existing parameter names for this signature too.
-- Signature: (integer, integer, integer, numeric)
-- Your DB already has p_prod_issues_24h (per the error), so match that exactly.
-- -----------------------------------------------------------------------------
create or replace function public.ms_production_status(
...
-- -----------------------------------------------------------------------------
-- 2) Rebuild v_production_decisions (NO CASCADE; safe because explain is dropped)
--    Must include suggested_action_code/text (append-only contract).
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3) Rebuild explain view aligned to SAME baseline logic
--    NOTE: explicit column list avoids duplicates (no c.* + c.suggested_action_code)
-- -----------------------------------------------------------------------------
...
calc as (
  select
    ...
    case
      ...
      when d.prod_issues_7d > 0
       and (d.prod_issues_24h / greatest(d.prod_issues_7d / 7.0, 0.25)) >= 1.5 then 'worsening'
      when d.prod_issues_7d > 0
       and (d.prod_issues_24h / greatest(d.prod_issues_7d / 7.0, 0.25)) <= 0.67 then 'improving'
      else 'stable'
